...
from src.app.entities import user_entity,rating_entity, trophies_entity, leisures_entity
import json
app = Flask(__name__)
...
def get_trophies_by_name(name):
    ...
    if not isinstance(trophy, dict):
        status = 404
        trophie = f'Error 404. Trophy with name: {name} not found'
...
def update_user_username(user_id,password):
    user = user_entity.User(user_id)
    status = 200
    if user is not None:
...
def update_trophy_name(trophy_id,text):
    trophy = trophies_entity.TrophyList(trophy_id)
    status = 200
    if trophy is not None:
